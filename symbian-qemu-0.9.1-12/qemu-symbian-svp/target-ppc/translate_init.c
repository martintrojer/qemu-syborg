...
/* E500 specific registers */
static void spr_read_spefscr (void *opaque, int gprn, int sprn)
{
    TCGv_i32 tmp = tcg_temp_new_i32();
    tcg_gen_trunc_tl_i32(tmp, cpu_gpr[gprn]);
    tcg_gen_ld_i32(tmp, cpu_env, offsetof(CPUState, spe_fscr));
...
{
    TCGv_i32 tmp = tcg_temp_new_i32();
    tcg_gen_st_i32(tmp, cpu_env, offsetof(CPUState, spe_fscr));
    tcg_gen_extu_i32_tl(cpu_gpr[gprn], tmp);
    tcg_temp_free_i32(tmp);
}
