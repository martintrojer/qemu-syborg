...
static void cpu_reset_model_id(CPUARMState *env, uint32_t id)
{
    ...
    case ARM_CPUID_ANY: /* For userspace emulation.  */
        ...
        set_feature(env, ARM_FEATURE_THUMB2EE);
        set_feature(env, ARM_FEATURE_DIV);
        set_feature(env, ARM_FEATURE_FP16);
        set_feature(env, ARM_FEATURE_THUMB2EE);
        break;
...
CPUARMState *cpu_arm_init(const char *cpu_model, void *dev)
{
    CPUARMState *env;
    uint32_t id;
    static int inited = 0;
...
void HELPER(set_cp15)(CPUState *env, uint32_t insn, uint32_t val)
{
    ...
    case 10: /* MMU TLB lockdown.  */
        ...
        break;
    case 12: /* Security extensions.  */
        /* This should probably be v6Z, and is technically optional on v7
           cores.  We only implement "non-secure" mode.  */
        if (!arm_feature(env, ARM_FEATURE_V7))
            goto bad_reg;
        if (crm != 0 || op2 != 0)
            goto bad_reg;
        env->cp15.c12_vbar = val & 0xffffffe0u;
        break;
    case 13: /* Process ID.  */
        switch (op2) {
        case 0:
